<%@ Page Title="" Language="C#" MasterPageFile="~/_MP_Front.master" AutoEventWireup="true" CodeFile="admin_company_lst.aspx.cs" Inherits="admin_company_lst" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphBody" runat="Server">
    <div class="panel panel-primary">
        <div class="panel-heading"><i class="fa fa-table"></i>&nbsp;Organization</div>
                            ...
                            <div class="col-xs-12 col-md-9 col-lg-10">
                                <div class="row">
                                    <div class="col-xs-12 col-md-8">
                                        ...
                                        <small runat="server" id="smtxtSeach" style="display: none">Found in facility / Sub-facility</small>
                                    </div>
                        ...
                        <table id="tblData" class="table dataTable table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="dt-head-center sorting" style="width: 35%;">Company Name</th>
                                    <th class="dt-head-center sorting" style="width: 15%;">Status</th>
                                    <th class="dt-head-center sorting" style="width: 15%;">Last Update</th>
                                    <th class="dt-head-center sorting" style="width: 15%;">Facility</th>
            ...
            AjaxCallWebMethod("LoadData", function (response) {
                if (response.d.Status == SysProcess.SessionExpired) {
                    ...
                    if (response.d.lstData != null && response.d.lstData.length > 0) {
                        var arrData = response.d.lstData;
                        //arrData = Enumerable.From(arrData).Where(function (w) { w=> w.SearchBy != "" }).ToArray();
                        arrData = Enumerable.From(arrData).Where(function (w) { return w.SearchBy != "" }).ToArray();
                        if (arrData.length > 0) {
                            $('small[id$=smtxtSeach]').show();
                        } else { $('small[id$=smtxtSeach]').hide(); }
                        ...
                        $.each(response.d.lstData, function (indx, item) {
                            $("td", row).eq(0).html('<input type="checkbox" id="ckbRow" class="flat-green cNoPrms" /> <input type="text" id="txtid" class="hidden" value="' + item.nCompID + '">' + nStartDataIndex + ".");
                            $("td", row).eq(1).html(item.sCompName);
                            $("td", row).eq(2).html(item.sStatus);
                            $("td", row).eq(3).html(item.sUpdate);
                            $("td", row).eq(4).html(item.sLinkFacility);
