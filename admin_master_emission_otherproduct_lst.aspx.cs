...
public partial class admin_master_emission_otherproduct_lst : System.Web.UI.Page
{
    ...
    public static TRetunrLoadData LoadData(CSearch itemSearch)
    {
        ...
        if (!UserAcc.UserExpired())
        {
            ...
            var TB_Effluent_Product = db.TEmission_OtherProduct_Stack.ToList(); //IsUse

            string _SQL = @" select nProductID
                              ,sName
                              ,CASE WHEN cActive = 'Y' THEN 'Active' ELSE 'Inactive' END as sStatus
                              ,dUpdate
                            from TM_Emission_OtherProduct
                            where cDel = 'N' " + sCon + @"
            ...
            foreach (var item in lstData)
            {
                ...
                item.sLink = "<a class='btn btn-warning' href='admin_master_emission_otherproduct_update.aspx?str=" + HttpContext.Current.Server.UrlEncode(STCrypt.Encrypt(item.nProductID.ToString())) + "'><i class='fa fa-edit'></i>&nbsp;Edit</a>";
                item.nIsUse = TB_Effluent_Product.Any(a => a.nOtherProductID == item.nProductID) ? TB_Effluent_Product.Where(a => a.nOtherProductID == item.nProductID).Count() : 0;
            }
    ...
    public static sysGlobalClass.CResutlWebMethod DeleteData(string[] arrValue)
    {
        ...
        if (!UserAcc.UserExpired())
        {
            ...
            db.TM_Emission_OtherProduct.Where(w => lstDelID.Contains(w.nProductID)).ToList().ForEach(x =>
            {
                x.cDel = "Y";
