...
public partial class admin_master_effluent_otherproduct_lst : System.Web.UI.Page
{
    ...
    public static TRetunrLoadData LoadData(CSearch itemSearch)
    {
        ...
        if (!UserAcc.UserExpired())
        {
            ...
            foreach (var item in lstData)
            {
                ...
                item.sLink = "<a class='btn btn-warning' href='admin_master_effluent_otherproduct_update.aspx?str=" + HttpContext.Current.Server.UrlEncode(STCrypt.Encrypt(item.nProductID.ToString())) + "'><i class='fa fa-edit'></i>&nbsp;Edit</a>";
                item.nIsUse = TB_Effluent_Product.Any(a => a.nProductID == item.nProductID) ? TB_Effluent_Product.Where(a => a.nProductID == item.nProductID).Count() : 0;
            }
    ...
    public static sysGlobalClass.CResutlWebMethod DeleteData(string[] arrValue)
    {
        ...
        if (!UserAcc.UserExpired())
        {
            ...
            db.TM_Effluent_OtherProduct.Where(w => lstDelID.Contains(w.nProductID)).ToList().ForEach(x =>
            {
                x.cDel = "Y";
