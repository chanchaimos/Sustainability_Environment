<%@ Page Title="" Language="C#" MasterPageFile="~/_MP_Front.master" AutoEventWireup="true" CodeFile="admin_master_waste_lst.aspx.cs" Inherits="admin_master_waste_lst" %>

<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="cphBody" runat="Server">
    <div class="panel panel-primary">
        <div class="panel-heading"><i class="fa fa-table"></i>&nbsp;<a href="admin_master_data_lst.aspx" style="color: white !important;">Master Data</a>&nbsp;<i class="fa fa-chevron-right"></i>&nbsp;Waste Disposal Method</div>
                ...
                <div class="col-xs-12">
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-xs-12 col-md-3 col-lg-2">
                                <a class="btn btn-primary btn-sm btn-block" href="admin_master_waste_update.aspx"><i class="fa fa-plus"></i>&nbsp;Create Waste</a>
                            </div>
                            <div class="col-xs-12 col-md-9 col-lg-10">
                                <div class="row">
                                    <div class="col-xs-12 col-md-8">
                                        <asp:TextBox ID="txtSearch" runat="server" CssClass="form-control" placeholder="Disposal Method Name ,Treatment and Disposal Codes"></asp:TextBox>
                                    </div>
                        ...
                        <table id="tblData" class="table dataTable table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="dt-head-center sorting" style="width: 20%;">Disposal Method Name</th>
                                    <th class="dt-head-center sorting" style="width: 35%;">Treatment and Disposal Codes</th>
                ...
                if (response.d.Status == SysProcess.SessionExpired) {
                    ...
                    if (response.d.lstData != null && response.d.lstData.length > 0) {
                        ...
                        $.each(response.d.lstData, function (indx, item) {
                            var ckbRow = item.nIsUse > 0 ? "" : "ckbRow";
                            var disabled = item.nIsUse > 0 ? "disabled" : "";

                            $("td", row).eq(0).html('<input type="checkbox" id="' + ckbRow + '" class="flat-green cNoPrms" ' + disabled + ' /> <input type="text" id="txtid" class="hidden" value="' + item.nWasteID + '">' + nStartDataIndex + ".");
                            $("td", row).eq(1).html(item.sTypeName);
                            $("td", row).eq(2).html(item.sCode + ' - ' + item.sName);
                            $("td", row).eq(3).html(item.sStatus);
                            $("td", row).eq(4).html(item.sUpdate);
